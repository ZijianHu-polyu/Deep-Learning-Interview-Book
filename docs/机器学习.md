...
最小化均方差，均方差有非常好的几何意义（对应了欧氏距离）

**求解$w$和$b$的过程（最小二乘法）**

1. 针对于均方差分别求$w$，$b$的偏导数；
2. 使得偏导数为0，计算$w$，$b$的闭式解。
注：可以拓展到多元变量（涉及计算矩阵逆，复杂度较高）

**正则化**

涉及到多元变量中，$w=(X^TX)^{-1}X^T y$，若要确定$w$的唯一解，需要$(X^TX)$满秩，现实任务中一般不满秩，可能会有多解。所以引入正则化项$\frac{\lambda}{2}||w||_2$，正则化项作为惩戒项，防止$w$系数中，出现过大或者过小项，让模型中系数的每一项都介于0-1之间，使得模型过于简单，使得模型更为稳定，泛化能力更高。
...
## 逻辑回归（LR）
1. 使用线性模型进行分类问题，只需找到一个单调可微的函数将分类任务的真实标记与线性回归模型的预测值联系起来。
2. 使用Sigmoid函数，可以看成线性回归的激活函数 $$y = \frac{1}{1 + \exp-(w^Tx+b)}$$ $$ln \frac{y}{1 - y} = w^x + b$$。
3. 一般通过极大似然估计来求解$w$和$b$的值，最后的形式就是交叉熵。是高阶可导凸函数，可以用梯度下降法来求解。
...
1.过采样，生成一些少类别的数据，例如SMOTE算法；
2.欠采样，booststrap采样一些多类别数据，使之和少类别数据量相当，使用ensemble方法进行集成；
3.给loss设置权值。

### 相关面经问题
**[为什么 LR 要使用 sigmoid 函数？]((https://blog.csdn.net/weixin_39881922/article/details/80366324))**
...
使用平方和损失的时候，当预测值靠近0或者1的时候，梯度很小，不利于收敛。而交叉熵是根据误差来计算的，误差越大，梯度越大
https://blog.csdn.net/liuweiyuxiang/article/details/100812623

**[LR 能否解决非线性分类问题？](https://www.zhihu.com/question/29385169)**
...
## 决策树
当我们拿到一套数据，数据当中会有不同的特征，在每个特征性下面会有不同的属性。我们可以某种度量标准，按照次序将特征作为节点，属性作为叉，构建一颗树作为我们的决策过程，这就是决策树。
常见的决策树主要分为三种，ID3, C4.5 和 CART。
+ ID3是以信息增益作为标准，优先选择信息增益大的特征作为节点进行分类。**但是可能会使样本量多的特征有更大的信息增益**。
+ 作为修正，C4.5以信息增益率为标准，用固有值normalize。**但是会使样本量少的特征有更大的信息增益率**，为了修正，在选择特征的过程中，首先先选择信息增益高于某一阈值的特征，再在这些特征里面选择信息增益率大的那一项。
+ CART 既可以用于分类也可以用于回归，以基尼指数作为划分标准，基尼指数（随机抽取两个样本不一样的概率）越小越好。CART是二叉树，每一叉仅代表是和否。

**决策树如何剪枝？**
减少数据噪声带来的过拟合风险。

分为预剪枝和后剪枝。
+ 预剪枝：将数据集划分为训练集和验证集，首先在训练集上面划分节点，用验证集验证该节点，如果划分节点能提升验证集的精确度，则划分。反之则不划分。**快，但容易欠拟合**。
+ 后剪枝：先划分好一整棵决策树。同样用验证集测试。从下到上剪枝叶子节点。如果删除某一叶子节点能提升验证集的精确度，则剪枝，反之则不剪枝。 **不容易过拟合，但是很慢**。

**决策树先剪枝还是后剪枝好？**

Trade off。预剪枝快，但是容易欠拟合，后剪枝慢，但是不容易欠拟合。

**决策树能否有非数值型变量？**

能，按照某一特征从最大到最小排列，划分点选择两相邻变量的平均值。将连续变量离散化。

## 梯度提升树（GBDT）
+ GBDT是属于集成学习boosting的一种算法。通过迭代拟合M个弱学习器，并且集成这M个弱学习器的效果，最后到达一个好的效果。在GBDT里面，弱学习器只能使用CART树，同时为了解决通用损失函数的拟合问题，我们使用负梯度来拟合损失的近似值。当这个损失函数为平方差损失时，这个负梯度就是我们说的残差。
+ 具体流程为：
  1. 假设我们一共有M个学习器，我们首先先初始化一个弱学习器，让它在当前的数据集上能够达到最好的效果。
  2. 进入迭代流程，在每轮迭代中，我们根据当前的强学习器计算损失函数的负梯度，当损失函数为平方差损失时，则为残差。然后我们用数据，和残差拟合出一颗回归CART树；
  3. 针对于这颗CART树，计算最佳的拟合值；
  4. 然后把这棵树加入到强学习中，最后更新强学习器。

**GBDT解决分类问题**
两种方法
1. 使用指数损失函数，此时GBDT退化成Adaboost算法
2. 使用sigmoid函数的损失函数

### 相关面经问题
**为什么要用CART树来作为基学习器**
1. CART树可解释强，泛化能力好，对于特征工程有所帮助；
2. 更少的参数化，不用调参；
3. 可以处理确实属性，不用做数据补全。

**GBDT的缺点**
1. 在高维数据上，因为CART模型训练低效，所以GBDT效果不佳。


## AdaBoost
+ Adaboost是属于集成学习boosting的一种算法。通过迭代拟合多个弱学习器（一般采用单层决策树），并将这多个弱学习进行合理的结合最后成为一个强分类器，能够达到一个很好的效果。
+ 具体流程为：
  1. 首先初始化样本权重，一般为$w_i = \frac{1}{m}$;
  2. 进入迭代流程，在每一轮迭代中，我们用数据集和样本权重训练一个弱学习器，分类误差是加权的。然后更新弱分类器的系数 $\alpha_k = \frac{1}{2} \log \frac{1 - e_k}{e_k}$, $e_k$为分类误差；
  3. 更新样本集的权重，赋予上一个弱学习器分错的样本更大的权重。
  4.结束迭代流程，构建最终的分类器，最终分类器为一个加性模型，是分类器权重和分类器共同作用的结果。

注：Adaboost算法中有两种权重，一种是数据的权重，另一种是弱分类器的权重。其中，数据的权重主要用于弱分类器寻找其分类误差最小的决策点，找到之后用这个最小误差计算出该弱分类器的权重（发言权），分类器权重越大说明该弱分类器在最终决策时拥有更大的发言权。

### 相关面经问题
**Adaboost的优缺点**
+ 优点：训练速度快，不容易发生过拟合；
+ 缺点：对于异常数据比较敏感，会给异常数据较大的权重，影响最后的分类效果。
...
## XGBoost
XGBoost是GBDT的一种高效的工程实现，同时在多方面优化了GBDT。具体的优化部分为：
1. **基分类器**：传统GBDT以CART作为基分类器，xgboost还支持线性分类器，这个时候xgboost相当于带L1和L2正则化项的逻辑斯蒂回归（分类问题）或者线性回归（回归问题）。
2. **导数信息**：传统GBDT在优化时只用到一阶导数信息，xgboost则对代价函数进行了二阶泰勒展开，同时用到了一阶和二阶导数。顺便提一下，xgboost工具支持自定义代价函数，只要函数可一阶和二阶求导。
3. **正则项**：XGBoost的目标函数加了正则项，使得学习出来的模型更加不容易过拟合。
4. **列抽样**：XGBoost支持列采样，与随机森林类似，用于防止过拟合。
5. **缺失值处理**：对树中的每个非叶子结点，XGBoost可以自动学习出它的默认分裂方向。如果某个样本该特征值缺失，会将其划入默认分支。
6. **Shrinkage（缩减）**，相当于学习速率（xgboost中的eta）。xgboost在进行完一次迭代后，会将叶子节点的权重乘上该系数，主要是为了削弱每棵树的影响，让后面有更大的学习空间。实际应用中，一般把eta设置得小一点，然后迭代次数设置得大一点。（补充：传统GBDT的实现也有学习速率）
7. **并行化**：XGBoost的并行，指的是特征维度的并行：在训练之前，每个特征按特征值对样本进行预排序，并存储为Block结构，在后面查找特征分割点时可以重复使用，而且特征已经被存储为一个个block结构，那么在寻找每个特征的最佳分割点时，可以利用多线程对每个block并行计算。
...
max_depth: 每棵子树的最大深度
min_child_weight: 子节点的权重阈值,如果一个结点分裂后，它的所有子节点的权重之和都大于该阈值，该叶子节点才可以划分。
gamma: 最小划分阈值，指的是，对于一个叶子节点，当对它采取划分之后，损失函数的降低值的阈值。
subsample：是对训练的采样比例 
...
如果我们把损失函数通过泰勒展开，那么我们会得到很多的高次项，而也正是因为这些高次项，所以容易导致过拟合，那么我们就可以对高次项进行惩罚，让它尽可能的小或者对整体的贡献尽可能的小，如此一来就可以防止过拟合啦
